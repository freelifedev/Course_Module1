...
# load the image, convrt it to grayscale, and blur it slightly
...
# finally, our constanc Cis subtracted from the mean calculation ( in this
# case 15)
thresh = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                               cv2.THRESH_BINARY_INV, 25, 15)
cv2.imshow("OpenCV Mean Thresh", thresh)

# personally, I prefer the scikit-image adaptive threshilding, it just
...
thresh = threshold_adaptive(image, 29, offset=5).astype("uint8") * 255
thresh = cv2.bitwise_not(thresh)
cv2.imshow("scikit-image Mean Thresh none blurr", thresh)
