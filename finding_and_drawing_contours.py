...
clone = image.copy()
# cv2.drawContours(clone, cnts, 2, (0,255,0), 2)
cv2.drawContours(clone, cnts[2], -1, (0,255,0), 2)
# cv2.drawContours(clone, [cnts[2]], -1, (0,255,0), 2)
print "Found {} contours".format(len(cnts))

# sho the output image
...
# show thee output image
cv2.imshow("All Contours", clone)


# re-clone the image and close alll open windows
cone = image.copy()
cv2.destroyAllWindows()

# loop over the contours individually
for c in cnts:
    # construct a mask by drawing onlythe current contour
